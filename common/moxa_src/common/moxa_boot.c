...
int do_run_mmc0_func(board_infos *board_info)
{
	...
	run_mmc_func(MMC0, board_info->dtbname, fs);
        //run_OS_test_func("T1(MP)", "uc5112");

	return -1;
}

int do_run_mmc0_mp_func(board_infos *board_info)
{
	int fs = 0;
	printf ("Boot to MMC0 SD card...\n");
...
int do_run_mmc_boot(board_infos *board_info)
{
	...
	return run_mmc_func(board_info->sys_mmc, board_info->dtbname, 2);
}
...
{
	char msg[MAX_SIZE_256BYTE] = {0};
	char boot_info[MAX_SIZE_64BYTE] = {0};
	char dtb_info[MAX_SIZE_64BYTE] = {0};
	char kernel_info[MAX_SIZE_64BYTE] = {0};
	int ret = 0;
        char *s1;
	...
	sprintf(kernel_info, "fatload mmc %d:1 0x82000000 imx7d-moxa-uc-8200.itb", boot_mmc);
        

        run_command (kernel_info, 0);

			/*sprintf(msg, "setenv bootargs mac=${ethaddr} sd=2 ver=3 console=ttymxc0,115200n8 root=/dev/mmcblk%dp2 \
				rw %s %s %s rootfstype=ext4 rootwait", fs_info, if_str, rb_str, fb_str);*/
	s1 = getenv("bootargs_set");

        if (s1 == NULL){
                sprintf(msg, "setenv bootargs console=ttymxc0,115200 root=/dev/mmcblk%dp2 rootwait rw", fs_info);
        } else {
                sprintf(msg, "setenv bootargs %s", s1);
        }
...
#endif
        //sprintf(boot_info, "bootz 0x81000000 - 0x83000000");
        sprintf(boot_info, "bootm 0x82000000#uc8200");
        run_command(boot_info, 0);
...
void run_OS_test_func(char * testmode, char * dtbname)
{
        ...
        if(testmode != NULL)
                sprintf(buf,"setenv bootargs mac=${ethaddr} sd=2 ver=3 console=ttyS0,115200 \
                        root=/dev/mmcblk0p2 rw net.ifnames=0 biosdevname=0 rootfstype=ext4 rootwait testmode=%s ",testmode);

        run_command(buf, 0);	
        
        if (strcmp("uc5101", dtbname) == 0)
                sprintf(buf, "fatload mmc 0:1 0x80000000 am335x-moxa-uc-5101.dtb");
        else if (strcmp("uc5102", dtbname) == 0)
                sprintf(buf, "fatload mmc 0:1 0x80000000 am335x-moxa-uc-5102.dtb");
        else if (strcmp("uc5111", dtbname) == 0)
                sprintf(buf, "fatload mmc 0:1 0x80000000 am335x-moxa-uc-5111.dtb");
        else if (strcmp("uc5112", dtbname) == 0)
                sprintf(buf, "fatload mmc 0:1 0x80000000 am335x-moxa-uc-5112.dtb");

        run_command(buf, 0);	

        run_command("fatload mmc 0:1 0x81000000 uimage", 0);
#if 1
        s1 = getenv("overlay_flag");
