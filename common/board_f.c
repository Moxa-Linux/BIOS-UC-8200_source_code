...
static int init_func_i2c(void)
{
	...
	i2c_init(CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);
        ...
        i2c_set_bus_num(2);
        buff[0] = 0x80;
        i2c_write(0x20, 0x06, 1, buff,1);
        buff[0] = 0x0f;
        i2c_write(0x20, 0x07, 1, buff,1);
        i2c_set_bus_num(1);
        pca953x_set_dir (PCA953X_ADDR1, 0xFFFF, 0x0600);//set direction to output

	pca953x_set_val (PCA953X_ADDR1, 0x00FF, 0x0033);//Default RS-232 Mode

	pca953x_set_val (PCA953X_ADDR1, 0xc000, 0x0000);

        for (i = 0; i < 110; i++)
                udelay(1000);

	pca953x_set_val (PCA953X_ADDR1, 0xc000, 0xc000);
