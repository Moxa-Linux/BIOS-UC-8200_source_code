...
#define ETHLOOP_HEAD_SIZE	42
static int ethloop_head_size ;
	...
	unsigned len)
{
	...
	for(i = 0; i < (ethloop_head_size + ETHLOOP_LEN); i++) {
		...
		printf ("%02x ",net_tx_packet[i]);
	}

	for(i = ethloop_head_size; i < (ethloop_head_size + ETHLOOP_LEN); i++) {
		if(i == ethloop_head_size)
...
int eth_loop_send(void)
{
	...
	eth_hdr_size = net_set_ether(pkt, ether, PROT_TEST);
	pkt += eth_hdr_size;
	net_set_udp_header(pkt, dest, 69, 69, ETHLOOP_LEN);
	pkt_hdr_size = eth_hdr_size + IP_UDP_HDR_SIZE;

	ethloop_head_size = pkt_hdr_size;

#ifdef ETH_LOOP_DEBUG
        printf("pkt_har_size = %d \n", pkt_hdr_size);

        for (i = 0; i < ETHLOOP_LEN; i++ ){
          printf("0x%02x ", net_tx_packet[i]);
          if ((i % 16) = 0)
              printf("\n");
        }
#endif
        /* given the packet value */
	for(i = 0; i < ETHLOOP_LEN; i++) {
		k = (i % 256);
		net_tx_packet[pkt_hdr_size + i] = k;
	}

	debug_cond(DEBUG_DEV_PKT, "sending UDP to %pI4/%pM\n",
		   &dest, ether);

	net_send_packet(net_tx_packet, pkt_hdr_size + ETHLOOP_LEN);	/* transmitted */

	printf("send loopbacak ok.\n");
	return 1;
}
