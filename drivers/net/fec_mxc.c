		...
		uint8_t regAddr)
{
	 ...
	 */
#if (defined IMX7_USE_SAME_MDIO) && (defined IMX_FEC_BASE)
	if (phyAddr == 2)
...
static void fec_reg_setup(struct fec_priv *fec)
{
	...
	if (fec->xcv_type == RGMII)
		...
		//rcntrl |= FEC_RCNTRL_RMII | FEC_RCNTRL_PROM;
		rcntrl |= FEC_RCNTRL_RMII ;

	writel(rcntrl, &fec->eth->r_cntrl);


        mibcntrl = readl(&fec->eth->mib_control);
        mibcntrl &= (0x0000000); 
	writel(mibcntrl, &fec->eth->mib_control);
}
